...
public class MongoDBTest {
	...
	public void setUp() throws Exception {
		...
		userCollection = mongoTemplate.getCollection("users");
	...
	public void testConditionQuery() {
		...
		// 查询age>=23
		List<DBObject> userList2 = userCollection.find(new BasicDBObject("age", new BasicDBObject("$gte", 23)))
				.toArray();
		print("        find age>=23: ");
		printList(userList2);

		// 查询age<=20
		List<DBObject> userList3 = userCollection.find(new BasicDBObject("age", new BasicDBObject("$lte", 20)))
				.toArray();
		print("        find age<=20: ");
		printList(userList3);

		// 查询age!=25
		List<DBObject> userList4 = userCollection.find(new BasicDBObject("age", new BasicDBObject("$ne", 25)))
				.toArray();
		print("        find age!=25: ");
		printList(userList4);
		...
		// 查询29>age>=20
		List<DBObject> userList7 = userCollection
				.find(new BasicDBObject("age", new BasicDBObject("$gte", 20).append("$lt", 29))).toArray();
		...
		fields.put("age", true);
		List<DBObject> userList11 = userCollection.find(query2, fields).skip(1).limit(1).toArray();
